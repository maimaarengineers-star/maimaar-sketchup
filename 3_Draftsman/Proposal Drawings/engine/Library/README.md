...
| Sliding door | `sliding_door/` | **drawers built, rendering, and VERIFIED AGAINST THE VECTORS** (`MAIMAAR_PEB_SlidingDoor.lsp`). Both issued sheets imported and measured; the pass overturned three things - there is NO diagonal brace (the D12 round bar is the floor rail), the stile grid is behind the panel and is not drawn in elevation, and MSPL-030 SDS-01 is a SINGLE leaf (its `3936` labels the wall the leaf parks over). Sample reproduces that sheet: leading strip and both panels exact, closers 11 mm out. Plan symbol traced from the approval drawing. NOT yet synced into the building drawings. |
