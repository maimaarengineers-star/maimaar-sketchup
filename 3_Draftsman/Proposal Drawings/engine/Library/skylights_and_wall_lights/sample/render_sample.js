'use strict';
// ── SAMPLE HARNESS — SKYLIGHTS & WALL LIGHTS ────────────────────────────────────────────────
//
...
// Out:  C:\maimaar_render\wl_<stamp>\WL_SAMPLE.png  +  .dwg
const fs = require('fs');
...
const WORK = 'C:/maimaar_render/wl_' + Date.now().toString(36);
...
const dwg = WORK + '/WL_SAMPLE.dwg';
const png = WORK + '/WL_SAMPLE.png';
...
const scr = [
  ...
  load('Library/skylights_and_wall_lights/MAIMAAR_PEB_LightPanel.lsp'),
  load('MAIMAAR_PEB_PDF.lsp'),
  '(peb-light-sample-from-file "' + q(WORK + '/' + dataFile) + '")',
  '(command "_.ZOOM" "_E")',
